...
export const parseExcelFile = async (file: File): Promise<ParsedExcelInfo> => {
  ...
  const mssvColumn = findColumnByKeywords(columns, ['mssv', 'ma so sinh vien', 'ma sinh vien', 'mã số sinh viên', 'mã sinh viên', 'student id', 'studentid']);
  const nameColumn = findColumnByKeywords(columns, ['ho va ten', 'ho ten', 'ten sinh vien', 'họ và tên', 'họ tên', 'tên sinh viên', 'full name', 'name', 'studentname']);
  const semesterColumn = findColumnByKeywords(columns, ['hoc ky', 'hoc ki', 'học kỳ', 'học kì', 'semester', 'termid']);
  const departmentColumn = findColumnByKeywords(columns, ['đv giảng dạy', 'đơn vị', 'đơn vị giảng dạy', 'don vi giang day', 'dv giang day', 'don vi', 'department', 'khoa', 'viện', 'vien', 'academicname']);
  const classCodeColumn = findColumnByKeywords(columns, ['mã lớp học', 'mã lớp', 'ma lop hoc', 'ma lop', 'class code', 'classid']);
  ...
  const examShiftColumn = findColumnByKeywords(columns, ['kíp thi', 'kip thi', 'ca thi', 'exam shift', 'shift', 'makip']);
  ...
  const dobColumn = findColumnByKeywords(columns, ['ngày sinh', 'ngay sinh', 'date of birth', 'dob', 'birthdate']);
...
export const parseGoogleSheetFromUrl = async (url: string): Promise<ParsedExcelInfo> => {
  ...
  const courseNameColumn = findColumnByKeywords(columns, ['tên học phần', 'tên hp', 'ten hoc phan', 'ten hp', 'môn học', 'mon hoc', 'course name']);
  const classNameColumn = findColumnByKeywords(columns, ['tên lớp', 'ten lop', 'class name']);
  const classExamCodeColumn = findColumnByKeywords(columns, ['mã lớp thi', 'ma lop thi', 'lớp thi', 'lop thi', 'class exam code', 'exam code', 'examid']);
  const examDateColumn = findColumnByKeywords(columns, ['ngày thi', 'ngay thi', 'exam date', 'date']);
  const examRoomColumn = findColumnByKeywords(columns, ['phòng thi', 'phong thi', 'exam room', 'room']);
  const examTimeColumn = findColumnByKeywords(columns, ['thời gian thi', 'thoi gian thi', 'giờ thi', 'gio thi', 'exam time', 'time']);
  const examShiftColumn = findColumnByKeywords(columns, ['kíp thi', 'kip thi', 'ca thi', 'exam shift', 'shift', 'makip']);
